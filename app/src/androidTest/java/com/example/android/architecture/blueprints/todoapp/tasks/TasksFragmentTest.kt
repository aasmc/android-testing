...
class TasksFragmentTest {
    ...
    fun clickTask_navigateToDetailFragment() = runBlockingTest {
        ...
        repository.saveTask(Task("TASK 2", "DESCRIPTION 3", true, "id2"))

        // GIVEN on the home screen
        val scenario = launchFragmentInContainer<TasksFragment>(Bundle(), R.style.AppTheme)
        // use Mockito's mock to create a mock of NavController
        val navController = mock(NavController::class.java)

        // make the created mock the fragment's navController
        scenario.onFragment {
            Navigation.setViewNavController(it.view!!, navController)
        }

        // WHEN - add the code to click on the RecyclerView item that has text TITLE 1
    ...
    fun clickAddTaskButton_NavigateToAddEditFragment() {
        ...
        // WHEN - add code to check that when clicked on FAB to add task
